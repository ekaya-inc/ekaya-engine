...
interface DatasourceSetupFlowProps {
  embedded?: boolean;
  onSaveSuccess?: () => void;
  onEmbeddedBackNavigationChange?: (handler: (() => void) | null) => void;
}

const DatasourceSetupFlow = ({
  embedded = false,
  onSaveSuccess,
  onEmbeddedBackNavigationChange,
}: DatasourceSetupFlowProps) => {
  ...
  useEffect(() => {
    if (!onEmbeddedBackNavigationChange) {
      return;
    }

    onEmbeddedBackNavigationChange(showSetup ? handleBackToSelection : null);
  }, [handleBackToSelection, onEmbeddedBackNavigationChange, showSetup]);
...
export default DatasourceSetupFlow;
