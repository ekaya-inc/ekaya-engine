/**
 * Pending Queries Page
 * Shows pending and rejected queries for AI Data Liaison app
 *
 * URL Structure:
 * - /pending-queries - Default (pending tab)
 * - /pending-queries/pending - Pending tab
 * - /pending-queries/rejected - Rejected tab
 * - /pending-queries/view/:queryId - View specific query (auto-selects tab based on query status)
 * - /pending-queries/edit/:queryId - Edit specific query
 * - /pending-queries/:status/new - Create new query
 */
...
function parseUrl(pathname: string, basePath: string): ParsedUrl {
  // Remove base path and get the pending-queries-specific part
  const queriesPath = pathname.replace(basePath, '').replace(/^\//, '');
  const segments = queriesPath.split('/').filter(Boolean);

  // Default values
  let status: QueryFilterType = 'pending';
  let queryId: string | null = null;
  let mode: ViewMode = 'list';

  if (segments.length === 0) {
    // /pending-queries - default to pending
    return { status: 'pending', queryId: null, mode: 'list' };
  }

  const first = segments[0];

  // Check for status-only paths: /pending-queries/pending, /pending-queries/rejected
  if (first === 'pending' || first === 'rejected') {
    status = first;

    // Check for /pending-queries/:status/new
    if (segments[1] === 'new') {
      mode = 'new';
    }
  ...
  // Check for /pending-queries/view/:queryId
  ...
  // Check for /pending-queries/edit/:queryId
  ...
  // Unknown path, default to pending
  return { status: 'pending', queryId: null, mode: 'list' };
}

const PendingQueriesPage = () => {
  ...
  const basePath = `/projects/${pid}/pending-queries`;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-text-secondary">
                Review, approve, or reject AI-suggested queries
              </p>
            </div>
          </div>
        </div>
...
export default PendingQueriesPage;
