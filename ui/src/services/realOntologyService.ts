...
function transformStatusToUI(
  ...
  // Calculate entity progress from Tier 2 tasks (each Tier 2 task = 1 entity)
  // Status bar shows entity count, work queue shows all tasks separately
  const tier2Tasks = taskQueue.filter((t) => t.name.startsWith('Tier 2:'));
  const completedTier2 = tier2Tasks.filter((t) => t.status === 'complete').length;
  const totalEntities = tier2Tasks.length;

  // Before Tier 2 tasks are enqueued, show task progress; after, show entity progress
  const showEntityProgress = totalEntities > 0;
  const progress: WorkflowProgress = {
    state: deriveWorkflowState(statusResponse),
    current: showEntityProgress ? completedTier2 : (statusResponse.completed_tasks ?? 0),
    total: showEntityProgress ? totalEntities : (statusResponse.total_tasks ?? 0),
...
class RealOntologyService {
  ...
  private async fetchAndUpdateStatus(): Promise<void> {
    ...
    try {
      ...
      // On error, set to idle state with the current questions
      this.status = {
        ...this.status,
        progress: {
  ...
  async startExtraction(): Promise<void> {
    ...
    try {
      // Call backend to start extraction
      await ontologyApi.extractOntology(this.projectId);
  ...
  /**
   * Restart the extraction (cancel + start)
   */
  ...
  async initialize(projectId: string): Promise<void> {
    ...
    try {
      ...
      if (statusResponse.workflow_id && !statusResponse.is_complete) {
        this.startPolling();
      } else if (statusResponse.workflow_id) {
        // Workflow exists but is complete - fetch final status once
...
export const realOntologyService = new RealOntologyService();
export default realOntologyService;
