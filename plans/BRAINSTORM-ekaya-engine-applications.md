...
PII Radar also monitors for **data minimization violations** — detecting when AI queries fetch more columns or rows than necessary for their stated purpose. If a recommendation engine runs `SELECT *` but only needs `product_id` and `category`, PII Radar flags the over-fetch. This directly supports GDPR Article 5(1)(c) data minimization requirements and reduces unnecessary sensitive data exposure to AI systems.
...
Query Audit Vault also assigns a **real-time risk score** to each AI query based on data sensitivity classification, query volume, accessing user/system identity, and whether human oversight exists for the downstream decision. Risk scores aggregate at the system level for compliance dashboards and trend analysis.
...
The automator also performs **unknown AI system detection** — fingerprinting all applications and services that query the database and alerting when an unrecognized system appears. This catches shadow AI: employees who connected ChatGPT to company data, vendor integrations nobody told security about, or new microservices querying production without registration. Every AI system touching company data should be known, documented, and periodically reviewed.
...
Beyond one-time DSARs, the agent includes **consent withdrawal propagation verification** (Right to be Forgotten). When a user exercises their right to withdraw consent or request deletion, the agent verifies that AI systems actually stop accessing that user's data. It runs periodic checks post-deletion to confirm no AI pipeline, recommendation model input, or analytics query still references the deleted user — providing proof of complete erasure across all AI data access paths.
...
The Collector also handles several additional compliance workflows:

- **DPIA automation:** When AI access patterns change materially (new data types accessed, new user segments, significant volume increase), automatically triggers a Data Protection Impact Assessment workflow and pre-populates it with observed data access patterns. Required by GDPR Article 35 and EU AI Act Article 9 for high-risk processing changes.
- **AI transparency disclosure generation:** Auto-generates documentation of what AI systems exist, what they do, and what data they consume. Serves as both internal AI literacy training (EU AI Act Article 4, effective February 2025, requires this for all companies regardless of risk tier) and external-facing regulatory transparency disclosure.
- **Regulatory incident reporting:** When AI accesses data outside policy, bulk PII exposure occurs, or anomalous decision patterns emerge, pre-drafts incident reports formatted for submission to supervisory authorities. EU AI Act Article 62 requires providers of high-risk AI to report serious incidents.
- **Human override verification:** Detects AI-driven decisions that had no human review. For high-risk AI systems (medical, financial, HR), EU AI Act Article 14 requires human oversight capability — this feature verifies it's actually happening in practice, not just theoretically available.
- **Post-market monitoring:** Continuous monitoring of AI system behavior after deployment, including data access drift, performance degradation signals, and compliance posture changes. EU AI Act Article 72 explicitly requires this for high-risk AI systems. Generates periodic monitoring reports for the Data Protection Officer (DPO) and regulatory authorities.
- **Framework mapping beyond SOC 2:** Extends evidence mapping to EU AI Act (Articles 9-72), GDPR (Articles 5-35), NIST AI RMF, and ISO 42001, in addition to the existing SOC 2, ISO 27001, and HIPAA coverage.

---

**7a. Consent Boundary Enforcer — AI access governance against user consent.** Continuously cross-references what data AI systems access against user consent records stored in the database (or synced from a consent management platform). If a user consented to "product recommendations" but an AI system reads their support tickets, billing history, or behavioral data beyond the consented scope, the Enforcer flags the boundary violation in real time. Stores consent-to-access mappings and violation history. Generates evidence that AI data access respects purpose limitation (GDPR Article 5(1)(b)) and consent boundaries. Most companies have consent management for web cookies but **zero governance over what AI systems access once data is in the database** — this closes that gap.

**7b. Bias & Representativeness Monitor — Demographic fairness for AI data inputs.** For companies using AI in high-risk domains (medical diagnosis, credit scoring, hiring, insurance), monitors whether the data being fed to AI models is demographically representative. Profiles data distributions across protected characteristics, detects skew and underrepresentation, and alerts when training or inference data deviates from configured fairness thresholds. EU AI Act Article 10 explicitly requires that training, validation, and testing datasets be "sufficiently representative" and free of bias for high-risk AI systems. Current approaches require manual statistical analysis by data scientists — this app automates continuous monitoring and generates audit-ready representativeness reports.
