...
- [ ] Read `src/services/ontologyApi.ts` to identify all exported async methods and response shapes
...
- [ ] Add tests for `pollStatus` polling logic: verify polling starts, calls the correct endpoint at each interval, stops on terminal status, handles errors
...
- [ ] Add tests for `transformEntityQueue`: provide representative input, assert correct output shape
...
- [ ] Add tests for `startPolling`: mock ontologyApi and timer functions (`vi.useFakeTimers()`), verify polling interval and stop behavior
- [ ] Add tests for `stopPolling`: verify timer is cleared and polling halts
- [ ] Run `cd ui && npx vitest run` and confirm all tests pass
...
- [ ] Add test: save operation calls `engineApi.updateAIConfig` with correct payload and shows success state
...
- [ ] Add test: error from any API call is displayed to the user
