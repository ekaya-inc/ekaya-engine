-- 003_schema.up.sql
-- Schema discovery tables: tables, columns, relationships
...
CREATE TABLE engine_schema_tables (
    ...
    PRIMARY KEY (id),
    CONSTRAINT engine_schema_tables_project_id_fkey FOREIGN KEY (project_id) REFERENCES engine_projects(id) ON DELETE CASCADE,
    CONSTRAINT engine_schema_tables_datasource_id_fkey FOREIGN KEY (datasource_id) REFERENCES engine_datasources(id) ON DELETE CASCADE
);
...
CREATE UNIQUE INDEX idx_engine_schema_tables_unique ON engine_schema_tables USING btree (project_id, datasource_id, schema_name, table_name) WHERE (deleted_at IS NULL);
...
CREATE TABLE engine_schema_columns (
    ...
    deleted_at timestamp with time zone,
    PRIMARY KEY (id),
    CONSTRAINT engine_schema_columns_project_id_fkey FOREIGN KEY (project_id) REFERENCES engine_projects(id) ON DELETE CASCADE,
    CONSTRAINT engine_schema_columns_schema_table_id_fkey FOREIGN KEY (schema_table_id) REFERENCES engine_schema_tables(id) ON DELETE CASCADE
);
...
CREATE INDEX idx_engine_schema_columns_joinable ON engine_schema_columns USING btree (schema_table_id, is_joinable) WHERE (deleted_at IS NULL AND is_joinable = true);
CREATE UNIQUE INDEX idx_engine_schema_columns_unique ON engine_schema_columns USING btree (schema_table_id, column_name) WHERE (deleted_at IS NULL);
...
CREATE TABLE engine_schema_relationships (
    ...
    deleted_at timestamp with time zone,
    PRIMARY KEY (id),
    CONSTRAINT engine_schema_relationships_confidence_check CHECK (confidence >= 0 AND confidence <= 1),
    CONSTRAINT engine_schema_relationships_project_id_fkey FOREIGN KEY (project_id) REFERENCES engine_projects(id) ON DELETE CASCADE,
    CONSTRAINT engine_schema_relationships_source_table_id_fkey FOREIGN KEY (source_table_id) REFERENCES engine_schema_tables(id) ON DELETE CASCADE,
    CONSTRAINT engine_schema_relationships_source_column_id_fkey FOREIGN KEY (source_column_id) REFERENCES engine_schema_columns(id) ON DELETE CASCADE,
    CONSTRAINT engine_schema_relationships_target_table_id_fkey FOREIGN KEY (target_table_id) REFERENCES engine_schema_tables(id) ON DELETE CASCADE,
    CONSTRAINT engine_schema_relationships_target_column_id_fkey FOREIGN KEY (target_column_id) REFERENCES engine_schema_columns(id) ON DELETE CASCADE
);
...
CREATE INDEX idx_engine_schema_relationships_rejection ON engine_schema_relationships USING btree (project_id, rejection_reason) WHERE (deleted_at IS NULL AND rejection_reason IS NOT NULL);
CREATE UNIQUE INDEX idx_engine_schema_relationships_unique ON engine_schema_relationships USING btree (source_column_id, target_column_id) WHERE (deleted_at IS NULL);

CREATE TRIGGER update_engine_schema_relationships_updated_at
    BEFORE UPDATE ON engine_schema_relationships
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- RLS for all schema tables
