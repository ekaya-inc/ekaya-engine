...
func Run(version string) error {
	...
	if staleStateRemoved {
		logger.Warn("Removed stale runtime state", zap.String("path", servercontrol.StatePath(cfg.ConfigPath)))
	}
	...
	controlStatus := servercontrol.Status{
		PID:        os.Getpid(),
	...
	controlServer, controlURL, err := servercontrol.Start(controlStatus, controlToken, func() {
		select {
		case shutdownRequests <- "control":
		default:
		}
	})
	if err != nil {
	...
	var runtimeCleanupOnce sync.Once
	cleanupRuntimeControl := func() {
		runtimeCleanupOnce.Do(func() {
			...
			if err := servercontrol.RemoveState(cfg.ConfigPath); err != nil && !errors.Is(err, os.ErrNotExist) {
				logger.Error("Runtime state cleanup error",
					zap.String("path", servercontrol.StatePath(cfg.ConfigPath)),
					zap.Error(err))
			}
		})
	}
	defer cleanupRuntimeControl()

	if err := servercontrol.WriteState(cfg.ConfigPath, &servercontrol.State{
	...
	go func() {
		...
		case source := <-shutdownRequests:
			logger.Info("Received shutdown request", zap.String("source", source))
		}
