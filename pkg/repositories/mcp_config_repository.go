...
func (r *mcpConfigRepository) Get(ctx context.Context, projectID uuid.UUID) (*models.MCPConfig, error) {
	...
	query := `
		SELECT project_id, tool_groups, audit_retention_days, created_at, updated_at
...
func (r *mcpConfigRepository) Upsert(ctx context.Context, config *models.MCPConfig) error {
	...
	query := `
		INSERT INTO engine_mcp_config (project_id, tool_groups, audit_retention_days, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $4)
		ON CONFLICT (project_id)
		DO UPDATE SET tool_groups = $2, audit_retention_days = $3, updated_at = $4`

	_, err = scope.Conn.Exec(ctx, query, config.ProjectID, toolGroupsJSON, config.AuditRetentionDays, now)
	if err != nil {
