...
const (
	minAutoGeneratedGlossaryTerms = 5
	maxAutoGeneratedGlossaryTerms = 8
)
...
Suggest 5-8 terms that are specific and meaningful for this domain.`
}
...
`)
	sb.WriteString("```\n\n")
	sb.WriteString("Suggest 5-8 domain-specific business terms based on the schema. Focus on quality over quantity.\n")
...
func (s *glossaryService) DiscoverGlossaryTerms(ctx context.Context, projectID uuid.UUID) (int, error) {
	return s.discoverGlossaryTerms(ctx, projectID, false)
}

func boundExampleGlossaryTerms(terms []*models.BusinessGlossaryTerm) ([]*models.BusinessGlossaryTerm, error) {
	if len(terms) < minAutoGeneratedGlossaryTerms {
		return nil, fmt.Errorf(
			"glossary auto-generate requires at least %d applicable terms, got %d",
			minAutoGeneratedGlossaryTerms,
			len(terms),
		)
	}

	if len(terms) > maxAutoGeneratedGlossaryTerms {
		return terms[:maxAutoGeneratedGlossaryTerms], nil
	}

	return terms, nil
}

func (s *glossaryService) discoverGlossaryTerms(ctx context.Context, projectID uuid.UUID, enforceExampleBounds bool) (int, error) {
	...
	// Filter out inapplicable terms based on table names
	filtered := filterInapplicableTerms(suggestions, tables)
	if enforceExampleBounds {
		filtered, err = boundExampleGlossaryTerms(filtered)
		if err != nil {
			return 0, err
		}
	}
