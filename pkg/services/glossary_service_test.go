...
func TestGlossaryService_DiscoverGlossaryTerms_ExampleBounds_LimitsToEightTerms(t *testing.T) {
	projectID := uuid.New()
	ctx := withTestAuth(context.Background(), projectID)
	...
	llmResponse := `{"terms": [
		{"term": "Metric 1", "definition": "Definition 1"},
		{"term": "Metric 2", "definition": "Definition 2"},
		{"term": "Metric 3", "definition": "Definition 3"},
		{"term": "Metric 4", "definition": "Definition 4"},
		{"term": "Metric 5", "definition": "Definition 5"},
		{"term": "Metric 6", "definition": "Definition 6"},
		{"term": "Metric 7", "definition": "Definition 7"},
		{"term": "Metric 8", "definition": "Definition 8"},
		{"term": "Metric 9", "definition": "Definition 9"},
		{"term": "Metric 10", "definition": "Definition 10"}
	]}`
	...
	datasourceSvc := &mockDatasourceServiceForGlossary{}
	adapterFactory := &mockAdapterFactoryForGlossary{}
	svc := NewGlossaryService(glossaryRepo, &mockColumnMetadataRepoForGlossary{}, nil, schemaRepo, &mockProjectServiceForGlossary{}, datasourceSvc, adapterFactory, llmFactory, nil, logger, "test").(*glossaryService)

	count, err := svc.discoverGlossaryTerms(ctx, projectID, true)
	require.NoError(t, err)
	assert.Equal(t, maxAutoGeneratedGlossaryTerms, count)

	terms, err := svc.GetTerms(ctx, projectID)
	require.NoError(t, err)
	require.Len(t, terms, maxAutoGeneratedGlossaryTerms)
}

func TestGlossaryService_DiscoverGlossaryTerms_ExampleBounds_RejectsFewerThanFiveTerms(t *testing.T) {
	projectID := uuid.New()
	ctx := withTestAuth(context.Background(), projectID)
	...
	count, err := svc.discoverGlossaryTerms(ctx, projectID, true)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "requires at least 5 applicable terms")
	assert.Equal(t, 0, count)

	terms, getErr := svc.GetTerms(ctx, projectID)
