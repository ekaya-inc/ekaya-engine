...
func (s *columnEnrichmentService) EnrichTable(ctx context.Context, projectID uuid.UUID, tableName string) error {
	...
	// Get FK info from column features (populated by column_feature_extraction service)
	fkInfo := s.extractFKInfoFromColumns(columns)
...
// extractFKInfoFromColumns extracts FK information from column features.
// ColumnFeatures are populated by the column_feature_extraction service in Phase 2.
func (s *columnEnrichmentService) extractFKInfoFromColumns(columns []*models.SchemaColumn) map[string]string {
	fkInfo := make(map[string]string)
	for _, col := range columns {
		features := col.GetColumnFeatures()
		if features != nil && features.IdentifierFeatures != nil && features.IdentifierFeatures.FKTargetTable != "" {
			fkInfo[col.ColumnName] = features.IdentifierFeatures.FKTargetTable
		}
	}
	return fkInfo
}
