...
type Client struct {
	projectID    uuid.UUID
	tunnelURL    string // full registration URL for the tunnel server
...
func NewClient(
	...
	connectURL, host := normalizeTunnelURL(tunnelURL)

	transport := http.DefaultTransport.(*http.Transport).Clone()
	if parsedLocalURL, err := url.Parse(localBaseURL); err == nil && parsedLocalURL.Scheme == "https" {
		// The tunnel relays back into the same engine instance. For internal HTTPS
		// targets we allow self-signed or public-host certificates on loopback.
		transport.TLSClientConfig = &tls.Config{InsecureSkipVerify: true} //nolint:gosec
	}

	return &Client{
		projectID:    projectID,
		tunnelURL:    connectURL,
		tunnelHost:   host,
...
func normalizeTunnelURL(rawURL string) (string, string) {
	trimmed := strings.TrimSpace(rawURL)
	parsed, err := url.Parse(trimmed)
	if err == nil && parsed.Host != "" {
		if parsed.Path == "" || parsed.Path == "/" {
			parsed.Path = "/tunnel/connect"
		}
		return parsed.String(), parsed.Host
	}

	connectURL := strings.TrimRight(trimmed, "/")
	if !strings.HasSuffix(connectURL, "/tunnel/connect") {
		connectURL += "/tunnel/connect"
	}
