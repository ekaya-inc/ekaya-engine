...
func Test_031_GlossaryDefiningSql(t *testing.T) {
	...
	// Verify unique index on (project_id, term)
	// Note: Migration 011 dropped ontology_id and changed this to (project_id, term)
	var uniqueExists bool
	err = engineDB.DB.Pool.QueryRow(ctx, `
		SELECT EXISTS (
			SELECT FROM pg_indexes
			WHERE tablename = 'engine_business_glossary'
			AND indexname = 'engine_business_glossary_project_term_unique'
		)
	`).Scan(&uniqueExists)
	require.NoError(t, err)
	assert.True(t, uniqueExists, "Unique index on (project_id, term) should exist")
